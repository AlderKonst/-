...
def players_add():
    ...
    if menu == '1':
        players = [my_card, my_card]
        players[0] = players[0].replace('о', 'o')
    elif menu == '2':
        players = [pc_card, pc_card]
        players[0] = players[0].replace('о', 'o')
    elif menu == '3':
...
def random_list():
    return [i for i in range(1, 91)]

# Создание двумерного списка с рандомными числами по возрастанию и пустотами
def card_list():
    num_lst = random_list()
    num_lst = [sorted(num_lst[:9]), sorted(num_lst[9:18]), sorted(num_lst[18:27])]
    card_lst = []
...
barrel_lst = random_list()  # Оставшиеся бочёнки
